...
def unlimitedCashRewards(driver, data, product_id):
    try:
        
        driver.get('https://www.bankofamerica.com/credit-cards/products/unlimited-cash-back-credit-card/')
        random_sleep(5)
        send_click(driver, '//a[@id="applyNow_engagement"]')
        ...
        # if not firstName or not lastName or not dob or not motherMaidenName or not ssn or not homeAddress or not city or not zipcode or not state or not email or not mobileNumber or not residencyStatus:
        ...
        if "middleName" in data.get("personal", {}):
            ...
            driver.find_element(By.CSS_SELECTOR, "#customerMiddleName").send_keys(Keys.TAB)
            random_sleep(1)
        else:
        ...
        driver.find_element(By.CSS_SELECTOR, "#phoneNumberMobile").send_keys(Keys.ARROW_UP)
        random_sleep(1)
        ...
        driver.find_element(By.CSS_SELECTOR, "#customerUsCitizenNo").send_keys(Keys.ARROW_UP)
        random_sleep(1)
        write_delay(driver, "#customerSocialSecurityNumber", ssn)
        
        random_sleep(3)
        send_click(driver, "//*[@id='customerDualCitizenshipNo']")
        send_click(driver, "#customerDualCitizenshipNo")
        random_sleep(1)
        
        # driver.find_element(By.CSS_SELECTOR, "#customerDualCitizenshipYes").send_keys(Keys.ARROW_DOWN)
        random_sleep(1)
        Select(driver.find_element(By.CSS_SELECTOR, "#customerCountryOfResidence")).select_by_index(1)
        ...
        driver.find_element(by=By.XPATH, value='//select[@id="employmentStatus"]').find_element(by=By.XPATH, value=f'.//option[@value="{employmentTypeDict.get(employmentType)}"]').click()
        random_sleep(1)

        send_click(driver, '//select[@id="occupation"]')
        random_sleep(1)
        driver.find_element(by=By.XPATH, value='//select[@id="occupation"]').find_elements(by=By.TAG_NAME, value='option')[-1].click()
        random_sleep(2)
        try:tempdropdown=driver.find_elements(by=By.XPATH, value='//a[@data-aaclickevent="true"]')
        except:
            random_sleep(5)
            tempdropdown=driver.find_elements(by=By.XPATH, value='//a[@data-aaclickevent="true"]')
        ...
        random_sleep(1)
        driver.find_element(by=By.XPATH, value='//select[@id="incomeSource"]').find_elements(by=By.TAG_NAME, value='option')[1].click()
        random_sleep(1)
        ...
        random_sleep(100)
        # driver.save_screenshot(f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - BOFA Unlimited Cash Rewards.png")

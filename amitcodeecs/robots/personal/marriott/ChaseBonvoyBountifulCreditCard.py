...
def ChasebonvoyBountiful(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=H8GS&action=guest#/origination/cardDetails/index/initiateConFullApp;cellCode=61DS;cfgCode=FULLAPPCONCC;channel=C30;sourceCode=H8GS;AOC=5056;RPC=0541;combo=N;params=,,,no,no,,,83d4dcfa-f7ec-4c1c-b164-96a3f9d9bf42"
        )
        # send_click(
        ...
        send_click(
            driver,
            '//*[@id="select-primaryIncomeSourceName-select-validate"]'
        )
        random_sleep(1)
        retry_count = 0
        while retry_count < 600:
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase Marriott Bonvoy Bountiful Credit Card.png"
            )
        random_sleep(1)
        ...
        card_name = "Chase Marriott Bonvoy Bountiful Credit Card"

...
def ChasebonvoyBold(driver,data, product_id):
    try:
        # data = {
        ...
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=HJ76&action=guest#/origination/cardDetails/index/initiateConFullApp;cellCode=61DS;cfgCode=FULLAPPCONCC;channel=C30;sourceCode=HJ76;AOC=5056;RPC=0532;combo=N;params=,,,no,no,,,"
        )
        # send_click(
        ...
        send_click(
            driver, 
            '//*[@id="select-state-blx-residentialAddressBlock-select-validate"]'
        )
        random_sleep(2)
        retry_count = 0
        ...
        send_click(
            driver,
            '//*[@id="select-residenceOwnership-select-validate"]'
        )
        random_sleep(1)
        retry_count = 0
        while retry_count < 600:
        ...
        # Employment status Check krishna bhaiya
        send_click(
            driver,
            '//*[@id="option-primaryIncomeSourceName-select-validate-0"]'
        )
        random_sleep(1)
        retry_count = 0
        while retry_count < 600:
            try:
                select_element = driver.find_element(
                    by=By.XPATH, 
                    value='//*[@id="option-primaryIncomeSourceName-select-validate-0"]'
                )
                select = Select(select_element)
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase Marriott Bonvoy Bold Credit Card.png"
            )
        random_sleep(1)
        ...
        card_name = "Chase Marriott Bonvoy Bold Credit Card"

...
def chasePreferred(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=HJHB&action=guest&cellCode=61DS&combo=Y&flowVersion=REACT&AOC=6530&RPC=0444&cfgCode=FULLAPPCONCC#/origination/cardDetails/index/initiateConFullApp"
        )
        # send_click(
        #     driver, 
        ...
        try:
            ...
            # send_click(
            #     driver, 
            #     '//*[@id="skipAppCap.consumerCertificationsAgreement-Fieldset"]'
            # )
        except:
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase Sapphire Preferred Card.png"
            )
        random_sleep(1)

...
def wellsFargoChoicePrivilegesMastercard(driver, data, product_id):
    try:
        driver.get(
            "https://www.choicehotels.com/choice-privileges/earn-points/mastercard-credit-card?FPID=0124CEIXP10LTO&utm_source=WF&app_source=WELLS_FARGO_COM&locale=en_US&product_code=CC&subproduct_code=CP&sub_channel=SEO&vendor_code=G&cx_nm=CXNAME_CSMDUAL&referrerurl=creditcardswellsfargocom&lang=en&refdmn=www.google.com"
        )
        random_sleep(1)
        send_click(
            driver,
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Choice Privileges Mastercard.png"
            )
        random_sleep(1)

...
def WellsFargoBiltMastercard(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://id.biltrewards.com/login/enter-email/"
        )
        random_sleep(1)
        send_click(
            driver, 
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Bilt Mastercard.png"
            )
        random_sleep(1)
        ...
        card_name = "Wells Fargo Bilt Mastercard"

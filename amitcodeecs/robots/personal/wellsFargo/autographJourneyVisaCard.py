...
def autographJourneyVisaCard(driver, data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://creditcards.wellsfargo.com/autograph-journey-visa-credit-card/?sub_channel=SEO&vendor_code=G"
        )
        random_sleep(1)
        send_click(
            driver,
        ...
        try:
            send_click(
                driver, '//*[@id="yourInfoCommand"]/div[6]/div[5]/div/div/div/div/div')
        except:
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Autograph Journey Visa Card.png"
            )
        random_sleep(1)

        # Submit button
        send_click(driver, '//*[@id="ULWLRVXI"]')
        retry_count = 0
        while retry_count < 600:
            try:
                driver.find_element(
        ...
        card_name = "Wells Fargo Autograph Journey Visa Card"

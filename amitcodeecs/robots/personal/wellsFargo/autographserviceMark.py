...
# ---------------------------------------------------------------
# Wells Fargo Autographservice Mark Card
# ---------------------------------------------------------------

def autographserviceMark(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://creditcards.wellsfargo.com/autograph-visa-credit-card/?sub_channel=SEO&vendor_code=G"
        )
        random_sleep(1)
        send_click(
            driver, 
        ...
        try:
            send_click(
                driver, '//*[@id="yourInfoCommand"]/div[6]/div[6]/div/div/div/div/div')
        except:
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Autographservice Mark Card.png"
            )
        random_sleep(1)

        # Submit button
        send_click(driver, '//*[@id="OFNYAFVB"]')
        retry_count = 0
        while retry_count < 600:
            try:
                driver.find_element(
        ...
        card_name = "Wells Fargo Autographservice Mark Card"

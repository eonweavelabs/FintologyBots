...
def reflectCard(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://creditcards.wellsfargo.com/reflect-visa-credit-card/?sub_channel=SEO&vendor_code=B"
        )
        random_sleep(1)
        send_click(
            driver, 
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Reflect Card.png"
            )
        random_sleep(1)

        # Submit button
        send_click(driver, '//*[@id="ZVMHVTKX"]')
        retry_count = 0
        while retry_count < 600:
            try:
                driver.find_element(

...
def wellsFargoChoicePrivilegesSelectMastercard(driver, data, product_id):
    try:
        driver.get(
            "https://www.choicehotels.com/en-uk/choice-privileges/earn-points/mastercard-credit-card?FPID=0123CFIXP10000&utm_source=WF&app_source=WELLS_FARGO_COM&locale=en_US&product_code=CC&subproduct_code=CE&sub_channel=SEO&vendor_code=G&cx_nm=CXNAME_CSMPD&referrerurl=creditcardswellsfargocom&lang=en&refdmn=www.google.com"
        )
        random_sleep(1)
        send_click(
            driver,
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Choice Privileges Select Mastercard.png"
            )
        random_sleep(1)

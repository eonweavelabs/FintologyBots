...
def ChasepremierVisaCard(driver,data, product_id):
    try:
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=H8YB&action=guest#/origination/cardDetails/index/initiateConFullApp;cellCode=61DS80;cfgCode=FULLAPPCONCC_DISV3;channel=C30;sourceCode=H8YB;AOC=6272;RPC=0474;combo=N;params=,,,no,no,,,"
        )
        # send_click(
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase Disney Premier Visa Card.png"
            )
        random_sleep(1)
        ...
        card_name = "Chase Disney Premier Visa Card"

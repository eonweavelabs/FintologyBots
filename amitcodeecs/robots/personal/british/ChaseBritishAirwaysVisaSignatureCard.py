...
def ChaseairwaysVisaSignature(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=H6KS&action=guest&cellCode=6TKV&combo=Y&flowVersion=REACT&AOC=286&RPC=0508&cfgCode=FULLAPPCONCC#/origination/cardDetails/index/initiateConFullApp"
        )
        # send_click(
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase British Airways Visa Signature Card.png"
            )
        random_sleep(1)
        ...
        card_name = "Chase British Airways Visa Signature Card"

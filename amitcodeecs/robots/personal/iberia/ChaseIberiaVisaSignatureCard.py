...
def ChasevisaSignatureCard(driver,data, product_id):
    try:
        # data = {
        ...
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=H84W&action=guest&cellCode=61DS&combo=N&flowVersion=REACT&AOC=6660&RPC=0527&cfgCode=FULLAPPCONCC#/origination/cardDetails/index/initiateConFullApp"
        )
        # send_click(
        ...
        try:
           driver.execute_script('document.querySelector("#root > div > div.mds-container-fluid.mds-px-at-768-0.containerLock > div > div.mds-col-at-576-8.mds-col-at-768-8.mds-col-at-992-6.mds-col-12.mds-mt-4 > main > form > div > div:nth-child(18) > mds-fieldset").querySelector("div> div> div> mds-checkbox").shadowRoot.querySelector("div>div").click()' )
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase Iberia Visa Signature Card.png"
            )
        random_sleep(1)
        ...
        card_name = "Chase Iberia Visa Signature Card"

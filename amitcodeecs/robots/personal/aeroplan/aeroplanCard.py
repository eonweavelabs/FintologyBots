...
def aeroplanCard(driver,data, product_id):
    try:
        # data = {
        ...
        # }
        driver.get(
            "https://secure.chase.com/web/oao/application/card?sourceCode=HHGM&action=guest&cellCode=TKV&combo=Y&flowVersion=REACT&AOC=6664&RPC=0538&cfgCode=FULLAPPCONCC#/origination/cardDetails/index/initiateConFullApp"
        )
        # send_click(
        #     driver, 
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Chase Aeroplan Card.png"
            )
        random_sleep(1)

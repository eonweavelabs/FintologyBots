...
def wellsFargoSignifyBusinessCash(driver, data, product_id):
    try:
        driver.get(
        ...
        try:
            ...
            driver.save_screenshot(
                f"{data.get('preApproval').get('firstName')+data.get('preApproval').get('lastName')} - Wells Fargo Signify Business Cash.png"
            )
        random_sleep(1)
